...
However, the mongo db pvc displays pending as its status, the frontend deployment has crashed severally despite all the efforts i have made to try and correct that and the mongo db statefulset is yet to start. 

THe debugging measures takes are trying to alter the code according to research and resources i have found but nothing has changed. I have also made several alterations to the code itself in the client and backend code files in the folders but nothing has worked.

I have advanced some more on the debugging measures and it has mongo db run efectively and the persistent volume and persistent volume claim files successfully created.
