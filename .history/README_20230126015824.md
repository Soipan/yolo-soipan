...
To expose the created pods to the internet, i have implemented services tied to the affected pods that will facilitate its  internet access. The services used is node-service using load-balancer for the frontend and backend together with mongo service for the database
 ...
 A PersistentVolume (PV) is a piece of storage in the cluster that has been provisioned by an administrator or dynamically provisioned using Storage Classes. Tied to the database are .yaml files that facilitate for its  persistent volumes to further ensure no data loss. This also is done through facilitating for persistence volume claim that ensures that products input that will be taken as data is not lost.
