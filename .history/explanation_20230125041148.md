...
## 6. Google K8s Project Deployment expound 
 ...
 My project has StatefulSets implemented in it. A StatefulSet represents a set of pods unique, persistent and with a stable pod name tied to the mongo database that help in maintaining the state of the database by creating replicas that aid in its state maintenance incase one fails.
